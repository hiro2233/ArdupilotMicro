...
void APM2RCOutput::init(void* machtnichts) {
    ...
    //hal.gpio->pinMode(3,HAL_GPIO_OUTPUT); // CH_1 (PB6/OC1B)
    hal.gpio->pinMode(5,HAL_GPIO_OUTPUT); // CH_2 (PB5/OC1A)
    hal.gpio->pinMode(6,HAL_GPIO_OUTPUT); // CH_3 (PH5/OC4C)
    //hal.gpio->pinMode(9,HAL_GPIO_OUTPUT); // CH_4 (PH4/OC4B)
    //hal.gpio->pinMode(10,HAL_GPIO_OUTPUT); // CH_5 (PH3/OC4A)
    ...
    //hal.gpio->pinMode(5,HAL_GPIO_OUTPUT); // CH_4 (PH4/OC4B)
    ...
    //TCCR0A =((1<<WGM21));
    //TCCR0B = (1<<WGM22)|(1<<WGM21)|(1<<CS21);
    //OCR0A = 0xFF; // Init OCR registers to nil output signal
    //OCR0B = 0xFF;
    //OCR2C = 0xFFFF;
    //ICR2 = 40000; // 0.5us tick => 50hz freq
...
void APM2RCOutput::enable_ch(uint8_t ch) {
    switch(ch) {
    case 0: TCCR0A |= (1<<COM0B1); break; // CH_1 : OC1B
    case 1: TCCR0A |= (1<<COM0A1); break; // CH_2 : OC1A
    //case 2: TCCR0A |= (1<<COM0B1); break; // CH_3 : OC4C
    //case 3: TCCR0A |= (1<<COM0A1); break; // CH_4 : OC4B
    //case 4: TCCR1A |= (1<<COM4A1); break; // CH_5 : OC4A
...
void APM2RCOutput::write(uint8_t ch, uint16_t period_us) {
    ...
    switch(ch)
    {
    case 0:  OCR0B=pwm; break;  // out1
    case 1:  OCR0A=pwm; break;  // out2
    //case 2:  OCR0B=pwm; break;  // out3
    //case 3:  OCR0A=pwm; break;  // out4
...
uint16_t APM2RCOutput::read(uint8_t ch) {
    uint16_t pwm=0;
    switch(ch) {
    case 0:  pwm=OCR0B; break;      // out1
    case 1:  pwm=OCR0A; break;      // out2
    //case 2:  pwm=OCR0B; break;      // out3
    //case 3:  pwm=OCR0A; break;      // out4
